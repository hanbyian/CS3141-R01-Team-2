...
class TermsServiceTest {
    ...
    /**
     * Method under test: {@link TermsService#delTermByID(Long)}
     */
    @Test
    void testDelTermByID() {
        doNothing().when(termsRepository).deleteById((Long) any());
        termsService.delTermByID(1L);
        verify(termsRepository).deleteById((Long) any());
    }
     ...
     */
    @Test
    void testDelTermByID2() {
        doThrow(new IllegalStateException()).when(termsRepository).deleteById((Long) any());
        assertThrows(IllegalStateException.class, () -> termsService.delTermByID(1L));
        verify(termsRepository).deleteById((Long) any());
    }
}
